...
p_amount=200 #amount of primes in factor base
p_mod_amount=200 #amount of primes in the modulus
...
def launch(n,primeslist1,quad_base,z_plist):
    ...
    while z < workers:
        p=multiprocessing.Process(target=find_comb, args=(primeslist1,n,procnum,return_dict,hmap,quad_base,hmap2,z_plist)) 
        jobs.append(p)
        p.start()
        procnum+=1
        z+=1            
...
def gen_comb(collected,mod,z2,n,factor_base,ret_array,quad_base,hmap2):
    ...
    for idx in enumerated_product(*enum):
        ...
        eq3=-1
        y0=y0_temp
        x2=get_root(n,mod,y0,z2) 
        z_target=n/((x2)**2)
        z_target=math.floor(z_target/mod)
        z=z2+mod*z_target
        while i<10:
            z=z+i*mod
            ...
            if smooth_can_left == 0 or smooth_can_right ==0 or eq3 < 0 or z%eq3 == 0:
                i+=1
                continue
            ...
            if rem2 == 1 and rem == 1:
                ...
                if z > eq3:
                    red+=1
                    is_red=1
                if smooth_can_left not in  ret_array[0][0]:# and smooth_can_right not in ret_array[0][1] and factors_left not in ret_array[1][0] and factors_right not in ret_array[1][1]:
                   # print("Smooth_can bitlen: "+str(bitlen(abs(smooth_can_right//mod)))+" left z mod eq3: "+str(bitlen(abs(z%eq3))))
                    jac=[]
                    ...
                    ret_array[2].append(jac)
                    ret_array[3].append(is_red)
                    return red
            i+=1           
...
def extract_factors(N, relations, null_space,flist,quad_base,is_red_list):
    n = len(relations)
    counter=0
    ...
    for vector in null_space:
      ...
      #  factors_left_added=[]
        ...
        jac=[]
        for prime in quad_base:
            jac.append(jacobi(prod_left%prime,prime))
        print("prod_left: "+str(prod_left)+" prod_right: "+str(prod_right)+" factors_right: "+str(factors_right_added)+" jac: "+str(jac))#+" is_red: "+str(isred))
        sqrt_left=math.isqrt(prod_left)
        if sqrt_left**2 != prod_left:
            print("blah")
            continue
        if prod_left%N != prod_right%N:
            print("blah2")
            continue
...
def build_matrix(factor_base, smooth_nums, factors,quadlist):#,smooth_nums2,factors2,qchar):
    print("\n\n")
    ...
    while i < len(quadlist):
       # print("qchar[i]: ",qchar[i])
        j=0
        while j < len(quadlist[i]):
            if quadlist[i][j]==-1 or  quadlist[i][j] == 0:
                M2[j] |= ind
            
            j+=1
        ind = ind + ind
        i+=1
    offset=quad_char_amount
    ind=1
    ...
    offset=quad_char_amount+p_amount+2
    ind=1
...
def QS(n,factor_list,sm,flist,quadlist,quad_base,is_red_list):#,flist,zlist,eq_list):#,sm2,flist2,qchar,z_list,disc_list,quad_base,mod_list):
    global p_amount
  ...
  #  print(len(sm[0]))
    ...
    M2 = build_matrix(factor_list, sm, flist,quadlist)#,sm2,flist2,qchar)
    null_space=solve_bits(M2)
    f1,f2=extract_factors(n, sm, null_space,flist,quad_base,is_red_list)#:#,z_list,disc_list,qchar,quad_base,mod_list)
    if f1 != 0:
...
def find_comb(lists,n,procnum,return_dict,hmap,quad_base,hmap2,z_plist):
    red=0
    ret_array=[[[],[]],[[],[]],[],[]]#,[],[],[],[],[],[],[]]
    z=g_z
   ...
   # for z in z_plist:
        ...
        while i < p_mod_amount:
            ...
            red+=gen_comb(collected,mod,z2,n,factor_base,ret_array,quad_base,hmap2)
        ...
        if len(ret_array[0][0])>(p_amount*2+4+quad_char_amount):
            break
        z+=1
  #  print("Amount of mod reduced left side: "+str(red)+" out of: "+str(p_amount*2+4+quad_char_amount))
    print("[i]Starting linear algebra")
    test=QS(n,lists,ret_array[0],ret_array[1],ret_array[2],quad_base, ret_array[3])#,ret_array[2],ret_array[6],ret_array[7])   
...
def main():
    ...
    while k<1:
       ...
       # plist=[3,11]#[5,7,11,13,17,19,23]
        launch(n,plist,quad_base,z_plist)
        duration = default_timer() - start
