...
Update: Actually I have an idea for tomorrow. Calculating the divisor is very hard, it would be an area of research which would take time. However, if we can make it so our divisor value is vary small, which ends up being the case if the discriminant is square.. then it does not matter. So tomorrow I should have a look at cases, for example when the discriminant is 3*3*2  .. aka.. nearly square. What happens with this divisor value if it is almost square? Because if I can calculate those cases.. then that would be an historical breakthrough and a major improvement on existing algorithms. Lets see tomorrow.
