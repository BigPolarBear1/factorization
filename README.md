...
This runs a standard SIQS variant using residue sieving instead of manipulating the coefficients, while still retaining self-initialization once a B-smooth is found it calls into find_same, flips things around and tries to sieve for similar b-smooths, although that logic is left unimplemented. It does created a linear polynomial split, so we end up sieving with 3 parts, f(x), g(x) and h(x) and the values of g(x) and h(x) can be presieved independetly and paired up at an offset cost that's paid by f(x). These last few days I have been doing some thinking, how do I go from here? One thing I did notice is that linear polynomias g(x) and h(x), its polynomial values will encode residues for the factors of N. Further more, we also know that if factor of N are p and q, then p + q are basically.. euhm, square root residues? We can precompute valid solutions for p+q and p-q.. I've shown that before many times already in the last 3 years and it was one of the original premises of my research. And this... got me thinking even more... if I can drop the requirement that either g(x) or h(x) needs to factorize and replace it with legendre symbols instead... that would be a win.. it will look a lot like NFS.. but the difference being that we can combine seeing results from many more polynomials... not just polynomials generated by a skew (that "b" variable in nfs's sieving loop of Kleinjung's variant).
