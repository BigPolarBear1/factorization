...
So goal for tomorrow in build_database2interval() we are going to build sieve intervals for ALL quadratic coefficients that are valid for a modulus in one go and save them on disk. That way we can save on function calls for that quadratic coefficients that have the same tiling mod p and really just start using the GPU and vector addition to our advantage. The uploaded code is really bad, once I'm happy with the overall strategy I'm going to need to do a lot of optimization work. I can already see its potential though..
