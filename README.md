...
Update: I just did some thinking. What I should do first thing tomorrow is in QS_Variant (not the improved one) ... just have it very quickly build sieve intervals for different quadratic coefficients. Because not having to deal with lifting, these calculations can all be done instantly to derive roots and coefficients for different quadratic coefficients, lifting is kind of a pain I guess. Then once that is done, that should already see a big speed boost, I can continue what I was doing in Improved_QS_Variant.
