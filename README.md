...
Update: I have been doing analysis of my work. So it would appear, that we should find some quadratic which produces a square value and where zx+y is also square. And then use the hashmap to find some multiple of that where also all the jacobi/legendre symbols indicate squaredness. On paper that would work. Which would be great because then we can remove the entire quadratic character base from the linear algebra step. So let me attack this problem from that angle. If I can remove the quadratic character base.. then that is indisputably a breakthrough.  Plus finding that multiple which turns all the jacobi symbols square.. that too intuitively feels like something I should be able to solve seperatly with linear algebra.
