...
Update: Actually thinking about it, building smooths is feasible. The main challenge is building smooths with small quadratic coefficients. But we can add and remove factors from a smooth the change that. Actually, while I'm also investigating the above, let me also go hard on this angle one final time. Because these quadratics now have no more secrets to me, if it is at all possible, I can figure it out. This is the one thing I must completely exhaust before calling it quits.
