...
Update: Still thinking how to best sieve this. There is multiple possible approaches. Also we can always use the quadratic coefficient to shrink the polynomial value without increasing the size of the linear coefficient. So there is all these different moving parts we can use. But I'm still trying to think how to turn that into an actual algorithm. All the math is here now. Let me do some experimentation to reduce the polynomial value size with the quadratic coefficient. When I think about this, I could hypothetically just spent an hour finding quadratic coefficients that factor over a factor base and save them to disk... since this would be independent of the number we are factoring, this would be a one time computational cost. And then I would have a large range of quadratic coefficients to chose from to reduce the polynomial value without changing that linear coefficient. So as long as we keep the linear coefficient small enough.. we can use precalculated quadratic coefficients to reduce that size.. hmm.. I do like this idea very much. Implementing this will be my goal for tomorrow. 
