...
If we have a polynomial, for example ax^2+bx-Nk, where N is the factor being factorized. Then this function sieves x, k and b. Where x and b have a large factor incommon so that ax^2+bx is nearly always garantueed to factor. This gives us the ability to move around linearly. You can visualize this as moving a parabola generated by sieving the root left or right linearly (slightly displacing it). Now, the idea is that we can do target hunting for very specific factors. So I need ot change some things around, the starting point should be the roots of our modulus, comprised of the factors we try to find.. and we use this idea of using this linear offset to keep generating small smooth candidates. I'm still trying to figure out the details on that as it's quite tedious. The function does demonstrate the main idea... although we can also sieve the leading coefficient and even switch to polynomials of higher degree.
