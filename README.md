...
Update: Oops. I was overcomplicating stuff. Run with the above command generating a 400 bit modulus. It will just generate a as large as possible root for each quadratic coefficient 0 to 100_000_000 and print it if the resulting smooth candidate has less then 195 bits. This garantuees that the quadratic coefficient stays small (relative to the modulus that is). Now we simply further reduce the size of the root with known factors in the factor base.... let me think how I'm going to do that. I should probably construct a sieve interval... but not like we did before.. do it for the way we are currently iterating in the PoC... but to get that working I need to formalize the progression of that root value as we increase the quadratic coefficient. 
