...
Update: Fixed the PoC so it can factor again. All the logic is in generate_large_square(). Its going to generate smooths that are roughly the size of keysize/2. I think this setup is the best I can do. Next I need to further reduce that bitlength with known factors... for that I will need to implement sieving. However.. I need to sieve over the length of all quadratic coefficient. Not just for one quadratic coefficient like default SIQS does. I'll do some brain storming how this is best approached. So the current setup if we take 400 bits.. it will generate roughly smooth candidates of 200 bits.. the size of the quadratic coefficient doesn't really matter... because we filter those in advance so they factor over the factor base. We could in theory use much larger values there. I could for each valid quadratic coefficient add a small sieve interval.. and then just fill them out before generate_large_square. Plus I can also use that larger factor base, the one we arnt using for linear algebra for marking larger squares using lifting. I think it may just be a matter of making sure all those calculations are done fast enough. Let me construct some sieving tomorrow.

Oh yea, important observation. The advantage this has over default SIQS, is that we can generate smooths that are roughly keysize/2 for as long as we want (before dividing with any known factors or moduli). Thats not something you can do with just one quadratic coefficient like SIQS does.

Update: Already added a dummy placeholder called sieve(). I'll need to iterate over the primes there and calculate for each quadratic coefficient if they fall within the sieve interval. So thats just a matter of calculating that root for quadratic coefficient = 1 and then recalculating them for each quadratic coefficient using the math I figured out earlier. That should just be copy paste work. Let me try to finish tomorrow at the latest. Belgian police keeps harassing me and I may get formally arrest on monday (they are doing this on direction of the americans).
