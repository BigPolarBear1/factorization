...
Update: I have really quickly created a split in the linear algebra step. This now means that the left side of the congruence is the square root of the polynomial value generated by the quadratics. And the right side of the congruence is zx*(zx+y). Which is a little closer to what NFS is doing. I will now begin doing analysis on this setup and see if we can sieve mod m instead.. this would mean that we can reduce the constant N in zx^2+yx-N mod m .. which would be useful. And we can use jacobi symbols for primes outside of the modulus. I know this is all things I tried before, but I think, setting it up the way I am now, I may actually be able to pull it off now. Find this bridge to NFS finally. And since we would basically be sieving with small quadratics, it would be much better. 

Update: Ok. I did some thinking. Let me do an experiment. Lets just select a large prime, slightly smaller then N to start with.
Next we reduce the polynomial value modulo this prime and factorize this. But we keep zx*(zx+y) in the integers. And we add a couple of jacobi symbols to make sure the polynomial value is also a square residue for other primes.
As a basic test. Then I keep shrinking that initial large prime and see what issues occure and figure out how they can be solved. I think thats probably the most basic setup I can think of, either I get that to work and I can burrow deeper into that appraoch, or I cant get it to work, in which case I'll go back to messing around with the quadratic coefficient to change the residue of zx in zx+y, so we can get specific factorizations out of that.
