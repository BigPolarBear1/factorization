...
Update: Making some more progress today. I'll take a break for today I think. So right now, I'm just implementing some code to find the correct root and coefficients for the polynomial product in extract_factors(). It's almost finished. I'll finish the rest tomorrow. But I'm just doing it the way I know and that is calculating roots and coefficients mod p and lifting them. I'll have to check if there is some shortcuts we can use for that later. There probably is some way to use purely p-adic lifting there. But anyway... for now I just need to see what the roots and coefficients end up being for that product so it doesn't matter how I get there, once I can see the numbers I'll take it from there and figure out how to best implement my PoC.
