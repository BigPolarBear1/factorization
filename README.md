...
Update: I quickly fixed the large prime variation in both PoCs. I did notice that when using multiple quadratic coefficients with QS_Variant, it is barely pulling smooths from quadractic coefficients other then 1. Which doesn't make sense... so I will need to investigate tomorrow what is going on there. On average per quadratic coefficient roughly half the primes of the factor base should be used to mark the sieve interval... I'll need to make sure there is no bug there, because that would degrade performance immensely.
