...
Update: Doing some more thinking.. there is one thing I MUST ABSOLUTELY test first thing tomorrow, and thats just getting rid of that garantueed factor on the rightside of the congruence (generated by the polynomial with -nk appended) and just sieve coefficients that are multiples of the root.. I know how to minimize that value on the rightside now and select ideal sieving ranges for the coefficients.... i've experimented with something similar in the old binomial_sieve project.. but learned a few things now.. so might be worth revisiting.
