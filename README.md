...
Update: Added so it sieves by multiplier now. 

At line 789: start_xy=1

I need to change that so it uses a start value generated by generate_modulus. I'll change it in the coming days.

Update: Just thinking about this even deeper. So yes, we have more solutions per prime now. But, if we sieve one root at a time, each unique root mod p, will only have one matching solution. So to make use of the fact that we have a higher solution density, we need to take into account all roots mod p. And the way to do that is to use a 2d sieving setup. Where the height represents roots and the length represents x+y. 
