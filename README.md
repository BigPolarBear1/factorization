...
Then the ultimate test will be, given a 400-bit number... we can set mod_mul to a low enough value so that it generates smooth candidates (poly_val) below 40 bits. And then hopefully we can reduce the quadratic ccoefficient enough in size so we can factor it over a factor base without requiring a ridiciously sized factor base.
