...
cdef construct_interval(list ret_array,partials,n,primeslist,hmap,hmap2,large_prime_bound,primeslist2,small_primeslist):
   ...
   # while i < len(hmap):
    ...
    while mult < 10_000:
        ...
        while k_ind < 20:
            ...
            while a_ind < sievelen: 
                ...
                y_start=a-x_start
                z_start=1
                if z_start*x_start+y_start != a:
                    print("fatal2232")
                    sys.exit()
                local_factors2=list(local_factors2)
                prev=1
                u=0
                exp=1
                while u < len(seen_primes2):
                    ...
                    if y%k ==0 and abs(y//k)<sievelen:
                        y=y//k
                    #    print(str(y)+" a: "+str(a_ind)+" k: "+str(k))
                        if y < 0:
                            u+=1
                            continue
                
                        if z*x**2+(y*k)*x-n*k_sq != poly_val:
                            print("something went wrong")
                        zxy=z*x+(y*k)
                        pair=[[-k, x*z], poly_val, [[1 ,1]], zxy, [1], [-k,x], 1]
                        if poly_val!=0 and zxy !=0 and math.gcd(x,k)==1 and pair not in pair_list[y]:# and x < 1000:
                            lin_tracklist[y]+=1
                            pair_list[y].append(pair)
    ...
    z=1
    while z < 5:
        print("lin_tracklist: "+str(lin_tracklist))

        i=1
        while i < sievelen:
            if lin_tracklist[i]>1:
                if len(pair_list[i]) != lin_tracklist[i]:
                    print("error2928")
                    sys.exit()
                pairs_used=pair_list[i]
                f_x=[1,i,-n]
                ...
                M=sievelen
                pairs_free,R_p, logs, divide_leading, pow_div, B_prime = initialize_2(f_x, d, primelist, z)
                pairs_used.extend(pairs_free)
                f_prime = get_derivative(f_x)
                Q, k = initialize_3(n, f_x, f_prime, B, z)
                test=3 + len(primeslist)# + B_prime + len(Q) + len(divide_leading)
                if len(pairs_used)<(test//2):
                    i+=1
                    continue
        ...
        while i < sievelen:
            ...
            i+=1
        i=0
        while i < len(orig_pairlist):
            j=0
            while j < len(orig_pairlist[i]):
                old_pair=orig_pairlist[i][j]
                x=old_pair[5][1]
                
                k=-old_pair[5][0]
                k_sq=k**2
                oldzxy=old_pair[0][1]+i*k
                poly_val=old_pair[1]
                if 1*x**2+i*k*x-n*k_sq !=  poly_val:
                    print("fatal abort1")
                    sys.exit()
                ...
                if z*x**2+y*k*x-n*k_sq !=  poly_val:
                    ...
                    sys.exit()
                pair=[[-k, x*z], poly_val, [[1 ,1]], zxy, [1], [-k,x], 1]
                if y > 0 and y < sievelen:
                    pair_list2[y].append(pair)
                    lin_tracklist[y]+=1
                j+=1
            i+=1
        pair_list=pair_list2
   # print("poly_sm: "+str(poly_sm))
    return
