...
###To run: python3 run_qs.py -keysize 25 -base 50 -debug 1 -lin_size 10 -quad_size 100_000
...
g_lift_lim=0.5
thresvar=40000  ##Log value base 2 for when to check smooths with trial factorization. Eventually when we fix all the bugs we should be able to furhter lower this.
...
def lift_a(prime,n,co,z):
    z2=1
    k=0
    ret=[]
    new=[]
    r=get_root(n,prime,co,z) 
    if r==-1:
        return 0
...
def test_similar_smooth(lin,quad_co,ret_array,partials,large_prime_bound,totalmod,local,n):
...
def find_similar_smooth_2(quad_co,primes,n,hmap,indexmap,indexes,local,ret_array,partials,large_prime_bound,grays):
    ...
    while poly_ind < end:
        ...
        poly_ind+=1
        if test_similar_smooth(lin,quad_co,ret_array,partials,large_prime_bound,totalmod,local,n):
            return 1
    return 0

def find_similar_smooth(local_factors,lin_co,quad_co,quad_interval,n,hmap,indexmap,quad_interval_index,ret_array,partials,large_prime_bound,grays):
    ...
    while i < len(quad_interval):
        ...
        if hit == len(local_factors):
            sim_found+=find_similar_smooth_2(i+1,local_factors,n,hmap,indexmap,indexes,local,ret_array,partials,large_prime_bound,grays)
    ...
    while j < size:
        if interval[j] > threshold:
            ...
            if poly_val not in ret_array[0]:
                ...
                if find_similar_smooth(local_factors,co,quad_co,quad_interval,n,hmap,indexmap,quad_interval_index,ret_array,partials,large_prime_bound,grays):
                    sim_found=1
...
def find_comb(n,hmap,primeslist1,indexmap):
   ...
   # residue_map=[]
    ...
    while i < len(quad_interval):
        ...
        tnum = int(((2*n*4*(i+1))**mod_mul) / 1)#(lin_sieve_size//2))
