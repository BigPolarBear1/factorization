...
###To run: python3 run_qs.py -keysize 30 -base 50 -debug 1 -lin_size 100 -quad_size 1
...
def find_quads(local_factors,poly_val,lin):
    enum_lin=[]
    c=0
    while c < len(local_factors):
        enum_lin.append(local_factors[c])
        co1=lin%local_factors[c]
        co2=(-co1)%local_factors[c]
        enum_lin.append([co1,co2])
        c+=1
...
def get_roots_mod(div,mod):###JUST FOR DEBUGGING... REMOVE LATER.. also not the proper way to get roots 
    roots=[]
    i=0
    while i < mod:
        if i**2%mod == div%mod:
...
def find_similar_smooth(local_factors1,lin_co,quad_co,quad_interval,n,hmap,indexmap,quad_interval_index,ret_array,grays,poly_val):
    #To do: Complete this function... we should be able to get this to work without having to find a square multiple in the integers..
    mod_array,mod_array_single,res=factor(poly_val,quad_interval[quad_co-1])
    enum_lin=find_quads(mod_array,poly_val,lin_co)
    enum_quad2=[]
    enum_lin2=[]
    i=0
    while i < len(enum_lin):
        enum_lin2.append(enum_lin[i+1])
        i+=2

    for idx in enumerated_product(*enum_lin2):
        lin2=0
        quad=quad_co
        l=0
        while l < len(idx):
        ...
        res=new_val//poly_val
        if div_sqrt**2 == abs(div) :
            ...
            if new_lin != lin_co:
                ...
                if factor1 != 1 and factor1 != n:
                    print("Found factor using find_similar() function: ",factor1)
                    sys.exit()
    return 0
    ...
    while j < size:
        if interval[j] > threshold:
            ...
            if poly_val not in ret_array[0]:
                ...
                if len(local_factors)>1 and poly_val >0 and poly_val%2!=0:# and poly_val%2 !=0 and poly_val >0:
                    print("Found smooth, checking coefficients..  initial smooth factors: "+str(local_factors))
                    if find_similar_smooth(local_factors,co,quad_co,quad_interval,n,hmap,indexmap,quad_interval_index,ret_array,grays,poly_val):
                        sim_found=1
