...
###To run: python3 run_qs.py -keysize 25 -base 30 -debug 1 -lin_size 2 -quad_size 1
            ...
            if local_mod>1:
                ...
                while c < 2:#quad_sieve_size+1:
                    ...
                    while poly_ind < end:
                        ...
                        mod_found,sim_found=process_interval(ret_array,single_interval,n,quad,lin,partials,large_prime_bound,local_primes,threshold_map[i],local_mod,size,mod_found,quad_interval,hmap,indexmap,quad_interval_index,grays)
...
def find_quads(local_factors,hmap,indexmap,quad_interval_index,quad_interval,n):
    ...
    i=1
    while i < length:
        if quad_interval[i] in local_factors:
          ...
          #  print("hmap: ",hmap[index])
            ...
            while j < length2:
                quads[-1].append(hmap[index][j])
                cos=lift_b(quad_interval[i],n,hmap[index][j+1],hmap[index][j])
                co[-1].append(cos)
                j+=2
        i+=1
    return primes,quads,co
...
def find_similar_smooth(local_factors1,lin_co,quad_co,quad_interval,n,hmap,indexmap,quad_interval_index,ret_array,grays):
   ...
   # print("local_factors: ",local_factors)
    ...
    primes,quads,cos=find_quads(local_factors,hmap,indexmap,quad_interval_index[quad_co-1],quad_interval[quad_co-1],n)
    sim_found+=check_quads(primes,quads,cos,n,ret_array)


  
    if sim_found > len(local_factors): 
...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cdef process_interval(ret_array,int [::1] interval,n,quad_co,lin_co,partials, large_prime_bound,unsigned long long [::1] local_primes,int threshold,cmod,Py_ssize_t size,mod_found,quad_interval,hmap,indexmap,quad_interval_index,grays):
        ...
        if interval[j] > threshold:
            ...
            if poly_val not in ret_array[0]:
                ...
                if len(local_factors)>1:
                    ...
                    if find_similar_smooth(local_factors,co,quad_co,quad_interval,n,hmap,indexmap,quad_interval_index,ret_array,grays):
                        sim_found=1
