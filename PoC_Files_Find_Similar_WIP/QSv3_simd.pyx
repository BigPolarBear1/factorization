...
###To run: python3 run_qs.py -keysize 30 -base 50 -debug 1 -lin_size 2 -quad_size 1
            ...
            if local_mod>1:
                ...
                while c < 2:#quad_sieve_size+1:
                    ...
                    while poly_ind < end:
                        ...
                        mod_found,sim_found=process_interval(ret_array,single_interval,n,quad,lin,partials,large_prime_bound,local_primes,threshold_map[i],local_mod,size,mod_found,quad_interval,hmap,indexmap,quad_interval_index,grays,primeslist)
...
def find_quads(local_factors,hmap,indexmap,quad_interval_index,quad_interval,n,mod_array,poly_val):
    ...
    while i < length:
        c=0
        while c < len(local_factors):
            if quad_interval[i] == local_factors[c]:
                enum_quad.append(mod_array[c])
                enum_lin.append(mod_array[c])
                enum_quad.append([])
                enum_lin.append([])
          ...
          #  print("hmap: ",hmap[index])
                ...
                while j < length2:
                    quads[-1].append(hmap[index][j])
                    enum_quad[-1].append(hmap[index][j])
                    enum_quad[-1].append(hmap[index][j])
                    cos=lift_b(quad_interval[i],n,hmap[index][j+1],hmap[index][j],mod_array[c])
                    enum_lin[-1].append(cos[0])
                    enum_lin[-1].append(cos[1])
                   # print("cos: ",cos)
                    co[-1].append(cos)
                    j+=2
                break
    ...
    return primes,quads,co,enum_quad,enum_lin

def check_quads(primes,quads,cos,n,ret_array):
    max_try=10_000
    sim_found=0
    primes.insert(0,len(primes)+1)
    primes=array.array('Q',primes)
    i=0
    while i < len(cos):
        j=0
       # print("prime: ",primes[i+1])
        prime=primes[i+1]
        while j < len(cos[i]):
            l=0
            while l < len(cos[i][j]):
              ...
              #  print("co: ",cos[i][j][l])
                k=0
                while k < len(cos[i][j][l]):
                    co=cos[i][j][l][k]
                    temp_quad=quads[i][j]
                    it=0
                    while it < max_try:
                        while temp_quad%2==0: ##blah blah blah, worst code ever. I need to refactor all this shit eventually.
                            temp_quad+=mod
                        poly_val=co**2-4*n*temp_quad

                       
                        if poly_val % (prime**(l+1)) !=0:
                            print('error')
                
                        if math.log2(abs(poly_val)) - math.log2(mod) < 30:
                          #  print("poly_val: "+str(poly_val)+" : "+str(poly_val/mod))
                            local_factors,value=factorise_fast(poly_val,primes)
                          #  print("value: "+str(value)+" local_factors: "+str(local_factors)+" primes: "+str(primes))
                            if value != 1:
                                temp_quad+=mod
                                it+=1
                                continue
                            if poly_val not in ret_array[0]:
                                sim_found+=1
                                print("Found a similar smooth"+str(local_factors)+" co: "+str(co)+" quad: "+str(temp_quad))
...
def factor(num,factor_base):
    ##To do: Remove later. Just for testing purposes
    ret_list=[]
    single_ret_list=[]
    flist=[]
 ...
 #   for p in factor_base:
        ...
        if test != 1:
            ...
            single_ret_list.append(p)
        i+=1

    return ret_list,single_ret_list,num

def enumerated_product(*args):
    yield from itertools.product(*(range(len(x)) for x in args))

def find_similar_smooth(local_factors1,lin_co,quad_co,quad_interval,n,hmap,indexmap,quad_interval_index,ret_array,grays,poly_val,primeslist):
    primeslist.insert(0,len(primeslist)+1)
    primeslist=array.array('Q',primeslist)
    mod_array,mod_array_single,res=factor(poly_val,quad_interval[quad_co-1])

    size=lin_sieve_size2*2
    primes,quads,cos,enum_quad,enum_lin=find_quads(mod_array_single,hmap,indexmap,quad_interval_index[quad_co-1],quad_interval[quad_co-1],n,mod_array,poly_val)
    enum_quad2=[]
    enum_lin2=[]
    i=0
    ...
    for idx in enumerated_product(*enum_quad2):
        #print(idx)
        lin2=0
        quad=0
        l=0
        while l < len(idx):
            quad+=enum_quad2[l][idx[l]]
            lin2+=enum_lin2[l][idx[l]]
            l+=1
        quad%=poly_val
        lin2%=poly_val
        new_val=lin2**2-n*4*quad
        #if new_val%poly_val != 0:
           # print("error")
        div=new_val//poly_val

        div_sqrt=math.isqrt(abs(div))

       # mod_array,mod_array_single,res=factor(new_val,primeslist)
        if div_sqrt**2 == div :
            print("fonud a square")
            s=solve_lin_con(n,-lin2,div_sqrt)
            lin2+=n*s
            if lin2%div_sqrt!=0:
                print("error")
            new_lin=lin2//div_sqrt
            if new_lin != lin_co:
                   # print("found")
                factor1=gcd(new_lin+lin_co,n)
...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cdef process_interval(ret_array,int [::1] interval,n,quad_co,lin_co,partials, large_prime_bound,unsigned long long [::1] local_primes,int threshold,cmod,Py_ssize_t size,mod_found,quad_interval,hmap,indexmap,quad_interval_index,grays,primeslist):
    ...
    while j < size:
        if interval[j] > threshold:
            ...
            if poly_val not in ret_array[0]:
                ...
                ret_array[1].append(co)
                ret_array[2].append(local_factors)
                if len(local_factors)>1 and poly_val%2 !=0 and poly_val >0:
                    print("Looking for similar smooths, amount needed: "+str(len(local_factors)+1)+" initial smooth factors: "+str(local_factors))
                    if find_similar_smooth(local_factors,co,quad_co,quad_interval,n,hmap,indexmap,quad_interval_index,ret_array,grays,poly_val,primeslist):
                        sim_found=1
