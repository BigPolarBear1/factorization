...
def get_mod_root(mod_item,resmap,x):
    mod=mod_item[0]
    cfact=mod_item[1]
    indexes=mod_item[2]
    pl=[]
    ...
    while i < len(indexes):
        ...
        try:
            ...
            continue
        i+=1
    #print("pl: "+str(pl)+" x: "+str(x)+" mod: "+str(mod))
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef mark_interval(int [:] interval,pos,prime,log,size):
...
def find_same(primeslist,factors,y,n,primelist_f,smooth_list,factor_list,root_list,factor_list2):
   ...
   # if len(primeslist)<20:
    ...
    #    sys.exit()
   # small_base=copy.deepcopy(primeslist[1:20])
   # for p in small_base:
     #   if p not in factors:
       #     factors.append(p)
#    factors.extend(small_base)
    found=0
   # print("looking for: "+str(factors)+" binomial term: "+str(y))
    degree=2
    while degree < 5:
...
def find_same2(primeslist,factors,y,n,primelist_f,smooth_list,factor_list,root_list,factor_list2,degree,resmap,old_k):
    ...
    quad=1
    factors=list(factors)#primeslist1[1:]
    factors.sort()
    modlist=[]
    ...
    while i < len(primeslist):
        ...
        if factor in factors:
            if factor == -1 or factor == 2 or old_k%factor==0:
                i+=1
                continue
            modfac.append(factor)
            modulus*=factor
        ...
        i+=1
    mod_item=[modulus,modfac,modind]
    print("[i]Done building root map, modulus factors: "+str(mod_item))
    xc=1
    while xc < lin_sieve_size:
        ...
        kres,mod=get_mod_root(mod_item,resmap,x)
        k_start=(pval)//n
        ...
        k_interval=np.ones(quad_sieve_size,dtype=np.int16)
       # k_interval=array.array('i',[0]*quad_sieve_size)
        i=0
        while i < len(primeslist):
            factor=primeslist[i]
            if mod%factor ==0:# or factor > lin_sieve_size:
                i+=1
                continue
            try:
                xm=x%factor
                ...
                for re in res:
                   ...
                   # while j < len(k_interval):
                    log=round(math.log2(factor))
                    k_interval[s::factor]+=log
                       # j+=factor
            except Exception as e:
                i+=1
                continue
        ...
        np.putmask(k_interval, k_interval < threshold, 0)
        indexlist=np.nonzero(k_interval)

        indexlist_y=indexlist[0]
        ind=0
        length=len(indexlist_y)
        
        while ind < length:# length:  
            i=int(indexlist_y[ind])
            if i == 0:
                ind+=1
                continue              
            ...
            if value == 1 and value2 == 1:
                if factors1 not in factor_list:# and factors2 not in factor_list2:
                    ...
                    factor_list2.append(factors2)
                    print("[*]Smooth# "+str(len(smooth_list))+"/"+str(len(primeslist)*2+10)+" Poly: "+str(poly)+" x: "+str(x)+" pval/mod bits: "+str(bitlen(pval//mod))+" seen_primes: "+str(factors1)+" k: "+str(k))#+" seen_primes2: "+str(factors2)+" threshold: "+str(threshold)+" indicated threshold: "+str(k_interval[i])+" k center: "+str(k_start+(quad_sieve_size//2)))
                    if found > 500:
                        return found
            ind+=1
        xc+=1
          ...
          #  print("Bin term: "+str(y)+" pval: "+str(bitlen(pval))+" lside: "+str(lside)+" value1: "+str(value)+" value2: "+str(value2)+" seen_primes2: "+str(factors2)+" poly: "+str(poly))
            ...
            if value == 1 and value2 == 1:
                ...
                if pval not in smooth_list and factors1 not in factor_list:
                    ...
                    root_list.append(lside)
                    factor_list2.append(factors2)
                    print("[!]Smooth# "+str(len(smooth_list))+"/"+str(len(primeslist)*2+10)+" Poly: "+str(poly)+" x: "+str(x)+" pval: "+str(pval)+" pval bits: "+str(bitlen(pval))+" seen_primes: "+str(seen_primes)+" k: "+str(k)+" seen_primes2: "+str(factors2)+" k center: "+str(k_start+(quad_sieve_size//2)))
                    found+=find_same2(primeslist,factors1,y,n,primelist_f,smooth_list,factor_list,root_list,factor_list2,degree,resmap,k)
...
@cython.boundscheck(False)
#@cython.wraparound(False)
cdef process_interval(int [:] interval,y,n,k,degree,primelist_f,smooth_list,factor_list,root_list,factor_list2,int [:] interval2,primeslist):
    poly=binomial_coeffs_fast(y,degree)
    poly+=[(-n*k)]
    cdef Py_ssize_t i=0
    found=0
    while i < lin_sieve_size-2:
        if interval[i]>threshold:
            if interval2[i]!=1 or interval2[i+2]!=1:
                i+=1
                continue
           ...
           # verify_log(interval[i],seen_primes)
            ...
            if value == 1 and value2 == 1:
                if factors1 not in factor_list:# and factors2 not in factor_list2:
                    factors1=list(factors1)
                    factors1.sort()                    
                    print("Smooth# "+str(len(smooth_list))+"/"+str(len(primelist_f)*2+10)+" pval: "+str(pval)+" lside: "+str(lside)+" seen_primes: "+str(factors1)+" seen_primes2: "+str(factors2)+" index: "+str(i)+" poly: "+str(poly))
                    found+=1
                    smooth_list.append(pval)
                    ...
                    last=poly[-1]
                 #   h=0
                 #   while h < len(primeslist):
                    #    prime=primeslist[h]
                    #    poly_new[-1]=last*prime
                    #    pval=evaluate(poly_new,y*i*prime)
                    #    lside=pval+last*prime        

                   #     factors1, value,seen_primes,seen_primes_indexes=factorise_fast(pval,primelist_f)
                   #     factors2, value2,seen_primes2,seen_primes_indexes2=factorise_fast(lside,primelist_f)
                   #     print(" Adding "+str(prime)+" poly: "+str(poly_new)+" seen_primes: "+str(seen_primes)+" seen_primes2: "+str(seen_primes2))
                   #     if value == 1 and value2 == 1:
                   #         if factors1 not in factor_list:
                   #             factors1=list(factors1)
                   #             factors1.sort()                    
                   #             print("!!!!!Smooth# "+str(len(smooth_list))+"/"+str(len(primelist_f)*2+10)+" pval: "+str(pval)+" lside: "+str(lside)+" seen_primes: "+str(factors1)+" seen_primes2: "+str(factors2)+" index: "+str(i)+" poly: "+str(poly))
                  #              found+=1
                  ...
                  #      h+=1
                    found+=find_same(primeslist,factors1,y,n,primelist_f,smooth_list,factor_list,root_list,factor_list2)
...
def filter(qbase,size):

    interval2= array.array("i",[0]*size)
    i=0
    while i < size:

        factors1, value,seen_primes,seen_primes_indexes = factorise_fast(i,qbase) 
        if value != 1:
            i+=1
            continue
