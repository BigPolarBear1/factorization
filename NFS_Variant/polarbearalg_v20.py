...
def gen_comb(collected,mod,z,n,factor_base,ret_array,quad_base):
    ...
    for idx in enumerated_product(*enum):
        ...
        while i < 1: 
            ...
            test_square=y0**2-n*4*z
            while test_square%mod == 0:
                test_square//=mod

            y1=int((abs(mod*test_square))**0.5)
            

            print(" mod: "+str(mod)+" z: "+str(z)+" test_square: "+str(test_square)+" y0: "+str(y0%n)+" y1: "+str(y1%n)+" root: "+str(x)+" eq: "+str(eq_o%(mod**0.5)))#+" eq2: "+str(eq2)+" root2: "+str(x2))#+" new_eq: "+str(eq)+" new_y1: "+str(new_y1)+" x: "+str(x)+" x2: "+str(x2)+" adjusted_eq: "+str(adjusted_eq)+" adjusted_root: "+str(adjusted_root%n)+" adjusted_root2: "+str(adjusted_root2%n))#+" adjusted_root: "+str(adjusted_root)+" adjusted_eq: "+str(adjusted_eq)+" z_sqrt: "+str(z_sqrt)+" adjusted_co: "+str(adjusted_co))
            factor=gcd(y1+y0,n)
            if factor != 1 and factor != n:
                ...
                if (y1+y0)%n != 0:
                    print("fix me, not a square in the integes after dividing mod m")
            i+=1
...
def find_comb(lists,n,procnum,return_dict,hmap,quad_base):
    ...
    while z < z_max:
        ...
        while i < p_mod_amount:
            ...
            i+=1
        if gcd(z,mod)!=1 or (isPrime(z,5) != 1 and z!=1 or z == 2):# or z**0.5%1!=0:
            z+=1
            continue
        if mod ==1:
            z+=1
