...
###notes: use with python3 polarbearalg_v28.py -keysize 20
...
p_amount=6 #amount of primes in factor base
...
def gen_comb(collected,mod,z2,n,factor_base,ret_array,quad_base,hmap2):
    ...
    for idx in enumerated_product(*enum):
        ...
        while eq2<n:# and i < 1:
            ...
            eq2_sqrt=math.isqrt(eq2)
            if eq2_sqrt**2 == eq2:# and eq2 > n:
                counter=1
                while counter < 1000:
                    skip=0
                    if jacobi(n*counter,eq2_sqrt) == -1:  ###Here we can just use n*counter .. 
                        counter+=1
                        continue
                    testprimes=[907,911,919,929,937,941,947,953,967,971,977,983,991]
                    for prime in testprimes:
                        if jacobi(eq2**2+n*eq2*counter,prime) == -1: ###For other primes we have to use the full discriminant
                            skip=1
                            break
                    if skip == 1:
                        counter+=1
                        continue
                    print("multiplier k passed the legendre test: ",counter)
                    test_square=eq2**2+n*eq2*counter
                    test_sqrt=math.isqrt(test_square)
                    if test_sqrt**2==test_square:
                        test=gcd(test_sqrt+eq2,n)
                      #  if eq2 < n and counter > 1:
                        print("Found factor: "+str(test)+" y0: "+str(y0)+" x2: "+str(x2)+" eq2: "+str(eq2)+" mod: "+str(mod)+" i: "+str(i)+" z: "+str(z)+" eq/mod: "+str(eq/(mod))+" x: "+str(x)+" test_square: "+str(test_square)+" z multiple: "+str(counter))
                        sys.exit()
                    counter+=1
            i+=1
...
def extract_factors(N, relations, null_space,colist,zlist,eq_list):
    n = len(relations)
    counter=0
       ...
       # counter+=1
        ...
        for idx in range(len(relations)):
            ...
            if bit == 1:
                ...
                prod_left_added.append(relations[idx])
                z_added.append(zlist[idx])
                eq_added.append(eq_list[idx])
                eq_total*=eq_list[idx]
            idx += 1
        if check == 0:
            continue
        sqrt_left=math.isqrt(prod_left)
        if sqrt_left**2 != prod_left:
            print("ERROR ERROR ERROR ERROR REEREROEROEO")
        print("prod_left: "+str(prod_left)+" sqrt_left: "+str(sqrt_left))
        y0=prod_left
        start=sqrt_left
        test_square=y0**2+N*prod_left
        t=math.isqrt(abs(test_square))
        if t**2 == test_square:
            test=gcd(y0+t,N)

            if test != 1 and test != N:
                eq_total=eq_total/prod_left
                found_z=(abs(y0**2 - t**2))/N
                print("Found factors of "+str(N)+" factor1: "+str(test)+" factor2: "+str(N//test)+" added: "+str(prod_left_added)+" z_added: "+str(z_added)+" eq_added: "+str(eq_added)+" eq_total: "+str(eq_total)+" found_z: "+str(found_z/prod_left))
...
def QS(n,factor_list,sm,colist,flist,zlist,eq_list):#,sm2,flist2,qchar,z_list,disc_list,quad_base,mod_list):
    global p_amount
   ...
   # if len(sm) > g_max_smooths: 
      ...
      #  del colist[g_max_smooths:]
      #  del flist[g_max_smooths:] 
 
    M2 = build_matrix(factor_list, sm, flist)#,sm2,flist2,qchar)
    null_space=solve_bits(M2)
    f1,f2=extract_factors(n, sm, null_space,colist,zlist,eq_list)#:#,z_list,disc_list,qchar,quad_base,mod_list)
    if f1 != 0:
...
def find_comb(lists,n,procnum,return_dict,hmap,quad_base,hmap2):
   ...
   # if len(ret_array[0])>0:
     #   test=QS(n,lists,ret_array[0],ret_array[1],ret_array[2],ret_array[3],ret_array[4])#,ret_array[6],ret_array[7],ret_array[3],ret_array[5],ret_array[8],quad_base, ret_array[4]) 
 #   else:
    #    print("no smooths found")  
    return 0
