...
#-v13 Also modulo reduce the square on the left side, but when we do this, the modulus must be larger then N. In theory we can leave it in the integers, and remove it from the matrix aswell. But let me figure out how to complete reduce the discriminant mod m (right side) before I make that decision.
...
def QS(n,factor_list,sm,colist,flist,sm2,flist2,qchar,z_list,disc_list,quad_base,mod):
    global p_amount
...
def find_comb(lists,n,procnum,return_dict,hmap,quad_base):
    ...
    while z < z_max:
        ...
        z+=2
    test=QS(n,lists,ret_array[0],ret_array[1],ret_array[2],ret_array[6],ret_array[7],ret_array[3],ret_array[5],ret_array[8],quad_base,mod)   
    return 0
