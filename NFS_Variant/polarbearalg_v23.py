...
p_mod_amount=11 #amount of primes in the modulus
...
def find_comb(lists,n,procnum,return_dict,hmap,quad_base,hmap2):
    ...
    while z < z_max:
        ret_array=[[],[],[],[],[],[],[],[],[]]
        i=0
        if isPrime(z,5) != 1 and z!=1 or z == 2:
            z+=1 
        tmod=1
        while i < p_mod_amount:
            ...
            tmod*=mod
            i+=1
        eqsqrt=get_partials(tmod,ret_array[0])
        roots=get_partials(tmod,ret_array[1])
      #  print("roots: ",roots)
        roots_enum=[]
        b=0
        while b < len(roots):
            roots_enum.append(roots[b+1])
            b+=2

        for idx in enumerated_product(*roots_enum):
            root_temp=0
            tmod2=1
            l=0
            while l < len(idx):
                if roots_enum[l][idx[l]] != 0:
                    tmod2*=roots[l*2]
                root_temp+=roots_enum[l][idx[l]]
                l+=1 
            root_temp%=tmod2
            factor=gcd(root_temp+(tmod2**0.5),n)
