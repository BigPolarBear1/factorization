...
###notes: use with python3 polarbearalg_v10.py -key 4387 (or alternatively with -keysize 16)

####I am working on my own custom number field sieve implementation using reducible quadratic polynomials.

###To do:
#The quadratic character base is working now (WOHOOO!). Next I will have to figure out how to reduce the modulus (p_mod_amount from 6 to 5.. and eventually even smaller). I suspect that eventually I will need to do something similar to square root taking over a finite field. But atleast the quadratic character base is working now.
          ...
          #  print("vector: ",bit)
            if bit == 1:
               ...
               # g=gcd(z,mod)
                ...
                x=get_root(N,mod,y0,z) 
                y1=formal_deriv(y0,x,z)
                eq1_full=(z*x**2-x*y0)%mod1
                ja=jacobi(eq1_full%mod1,mod1)


                print("[Debug Info]Adding to left mod N: "+str(relations[idx]%N)+" adding to right mod N: "+str(relations2[idx]%N)+" y0: "+str(colist[idx])+" z: "+str(z_list[idx])+" discriminant: "+str(disc_list[idx])+" qchar: "+str(qchar[idx])+" root: "+str(x)+" eq1: "+str(eq1_full%N))#+" ja:"+str(ja))
