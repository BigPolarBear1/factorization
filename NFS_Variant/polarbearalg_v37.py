...
def launch(n,primeslist1):
    ...
    while z < workers:
        p=multiprocessing.Process(target=find_comb, args=(primeslist1,n,procnum,return_dict,hmap,hmap2)) 
        jobs.append(p)
        p.start()
        procnum+=1
        z+=1            
...
def gen_comb(collected,mod,z2,n,factor_base,ret_array,hmap2):
    enum=[]
    i=0
    ...
    for idx in enumerated_product(*enum):
        ...
        while eq3 < n:
            ...
            if z_new ==0:
                print("error")
            factors_left,rem=factorise_fast(z_new,factor_base)  ###Eventually we need to create a sieve_interval for this...
            if rem != 1:
                j+=1
                continue
            eq3=z_new*x**2-n
           ...
           # if mod == 169:
          #  print(" x: "+str(x)+" z: "+str(z_new)+"  mod: "+str(mod)+" eq3/mod "+str(eq3/mod)+" eq3: "+str(eq3)+" bitlen: "+str(bitlen(abs(eq3)))+" bitlen of x squared: "+str(bitlen(abs(x**2))))
            smooth_can_left=z_new*x**2
            smooth_can_right=eq3
            if smooth_can_right ==0:
                j+=1
                continue
            
            factors_right,rem2=factorise_fast(smooth_can_right,factor_base)
            if rem2 == 1:
                if smooth_can_left not in ret_array[0][0]:# and smooth_can_right not in ret_array[0][1]:
...
def extract_factors(N, relations, null_space,flist):
    ...
    for vector in null_space:
     ...
     #   print("prod_left: "+str(prod_left)+" prod_right: "+str(prod_right)+" factors_left: "+str(factors_left_added)+" factors_right: "+str(factors_right_added))
        ...
        if sqrt_left**2 != prod_left or sqrt_right**2 != prod_right:
            print("WTF")
...
def QS(n,factor_list,sm,flist):
  ...
  #  print(len(sm[0]))
    ...
    f1,f2=extract_factors(n, sm, null_space,flist)
    if f1 != 0:
...
def find_comb(lists,n,procnum,return_dict,hmap,hmap2):
    ret_array=[[[],[]],[[],[]]]#,[],[],[],[],[],[],[]]
    z=g_z
    ...
    while z < z_max:
        ...
        while i < p_amount:
            ...
            gen_comb(collected,mod,z2,n,factor_base,ret_array,hmap2)
        ...
        if len(ret_array[0][0])>smooth_len:
            ...
            print(smooth_len)
        if len(ret_array[0][0])>p_amount*2.1:
            break
        z+=1
