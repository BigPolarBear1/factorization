...
p_amount=100 #amount of primes in factor base
p_mod_amount=100 #amount of primes in the modulus
g_enable_custom_factors=0
g_p=107
g_q=41
quad_char_amount=500
...
def gen_comb(collected,mod,z2,n,factor_base,ret_array,quad_base,hmap2):
    ...
    for idx in enumerated_product(*enum):
        ...
        z_target=n/((x2)**2)
        z_target=math.floor(z_target/mod)#-10#000
        z=z2+mod*z_target
        while eq3<n and i < 1000:
            z=z+i*mod
            ...
            if smooth_can_left == 0 or smooth_can_right ==0 or eq3 < 0 or z%eq3 == 0:
                i+=1
                continue
            factors_left,rem=factorise_fast(z%eq3,factor_base)

            factors_right,rem2=factorise_fast(smooth_can_right,factor_base)
            if rem2 == 1 and rem == 1:
                if smooth_can_left not in  ret_array[0][0]:
                    jac=[]
...
def extract_factors(N, relations, null_space,flist,quad_base):
    n = len(relations)
    counter=0
    ...
    for vector in null_space:
      ...
      #  factors_left_added=[]
        ...
        print("prod_left: "+str(prod_left)+" prod_right: "+str(prod_right)+" factors_right: "+str(factors_right_added)+" jac: "+str(jac))
...
def QS(n,factor_list,sm,flist,quadlist,quad_base):#,flist,zlist,eq_list):#,sm2,flist2,qchar,z_list,disc_list,quad_base,mod_list):
    global p_amount
  ...
  #  print(len(sm[0]))
    ...
    f1,f2=extract_factors(n, sm, null_space,flist,quad_base)#:#,z_list,disc_list,qchar,quad_base,mod_list)
    if f1 != 0:
...
def find_comb(lists,n,procnum,return_dict,hmap,quad_base,hmap2):
    ...
    while z < z_max:
        ...
        if len(ret_array[0][0])>smooth_len:
            ...
            print(str(smooth_len)+" / "+str(p_amount+quad_char_amount))
        if len(ret_array[0][0])>(p_amount+quad_char_amount):
            break
        z+=1
    print("[i]Starting linear algebra")
    test=QS(n,lists,ret_array[0],ret_array[1],ret_array[2],quad_base)#,ret_array[2],ret_array[6],ret_array[7])   
