...
###notes: use with python3 polarbearalg_v07.py -key 4387, refer to paper for the math.

####I am working on my own custom number field sieve implementation using reducible quadratic polynomials.

###To do:
##1. We don't want to use big moduli, now we leverage all our number theory to use small moduli instead...
...
def gen_comb(collected,mod,z,n,factor_base,ret_array):
    print("Checking z: "+str(z)+" mod: "+str(mod))
    enum=[]
    i=0
    ...
    for idx in enumerated_product(*enum):
        ...
        while i < 1:  ###Idea: We can also increase the modulus on the quadratic coefficient... i'll need to experiment in the future
            ...
            #if eq1_full == 0 or eq1_b_full == 0:#//mod)%mod == 0:
             #   i+=1
              #  continue
            factors,rem=factorise_fast(square1,factor_base)
            factors2,rem2=factorise_fast(square2,factor_base)#//mod)%mod,factor_base)
            if rem == 1 and rem2 == 1:
          ...
          #  eq1_b=eq1_b_full%mod
            ...
            i+=1
            if square1**0.5%1==0 and square2**0.5%1==0:
                sqr1=square1**0.5
                sqr2=square2**0.5
                print("[*]Found two squares: y0: "+str(y0)+" square1: "+str(square1)+" square2: "+str(square2)+" sqr1**0.5: "+str(square1**0.5)+" sqr2**0.5: "+str(square2**0.5)+" gcd: "+str(gcd(sqr1+sqr2,n)))
...
def extract_factors(N, relations, null_space,relations2):
        ...
        for idx in range(len(relations)):
          ...
          #  print("vector: ",bit)
            if bit == 1:
                ...
                print("adding to right: ",relations[idx])
                print("adding to left: ",relations2[idx])
            idx += 1
...
def QS(n,factor_list,sm,xlist,flist,sm2,flist2):
    ...
    f1,f2=extract_factors(n, sm, null_space,sm2)
    if f1 != 0:
...
def find_comb(lists,n,procnum,return_dict,rstart,rstop,hmap):
    ...
    while z < z_max:
        ...
        gen_comb(collected,mod,z,n,factor_base,ret_array)
   ...
   # if len(ret_array[0])>0:
     ...
     #   while g < len(ret_array[0]):
          ...
          #  print("alg1: "+str(ret_array[0][g])+" y0: "+str(ret_array[1][g])+" factors1: "+str(ret_array[2][g])+" z: "+str(ret_array[3][g])+" mod: "+str(ret_array[4][g])+" alg2: "+str(ret_array[6][g])+" factors2: "+str(ret_array[7][g]))
        #    g+=1
