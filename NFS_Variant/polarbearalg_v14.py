...
###notes: use with python3 polarbearalg_v13.py -keysize 14

####I am working on my own custom number field sieve implementation using reducible quadratic polynomials.

###To do:
#I managed to fix the linear algebra step now... all seems to be working very similar to NFS now... I'll have to keep imrpoving and optimizing everything now. May also need some type of coefficient adjustment when we have two squares that arn't congruent mod N... I know how to go about doing that... I'll fix it soon.


#changelog (from v08 onward):
#-v14 jacobi symbols now calculated on the full squared coefficient not just mod m
#-v13 Simplified the matrix. If we will leave left side in the integers, there is no reason to have it in the matrix.
#-v12 One side is now fully in the integers, this seems to yield better results during linear algebra step.
#-v10 Just fixed a few minor things for clarity
#-v09 resolved issues with the quadratic character base
...
p_mod_amount=5 #amount of primes in the modulus
           ...
           # factors,rem=factorise_fast(square1,factor_base)
            ...
            if rem2 == 1:
                ...
                while q < len(quad_base):
                    ja_ys=jacobi((y0**2-n*4*z)%quad_base[q],quad_base[q])
                    if ja_ys == -1:
                         qchar.append(-1)
                    elif ja_ys != 0:
                        ys=tonelli((y0**2-n*4*z)%quad_base[q],quad_base[q])
                        if solve_quadratic_congruence(z%quad_base[q],(ys)%quad_base[q],(-n)%quad_base[q],quad_base[q]) == -1:
                            qchar.append(-1)
                        else:
                            qchar.append(0)
                    else:
...
def extract_factors(N, relations, null_space,relations2,colist,z_list,disc_list,qchar,quad_base,mod):
    print("[i]Modulus: "+str(mod)+" quad_base: "+str(quad_base))
    n = len(relations)
    mod1=19
    for vector in null_space:
        ...
        for idx in range(len(relations)):
          ...
          #  print("vector: ",bit)
            if bit == 1:
                ...
                prod_left_mod *=relations[idx]%mod
               ...
               # g=gcd(z,mod)
                ...
                if relations[idx]%N != relations2[idx]%N:
                    ...
                    wrap=1
                print("[Debug Info]Adding to left mod N: "+str(relations[idx]%N)+" adding to right mod N: "+str(relations2[idx]%N)+" y0: "+str(colist[idx])+" z: "+str(z_list[idx])+" discriminant: "+str(disc_list[idx])+" qchar: "+str(qchar[idx])+" prod_left_mod: "+str(relations[idx]%mod))#+" ja:"+str(ja))
        ...
        print("sqrt_right: "+str(sqrt_right%N)+" sqr_right: "+str(sqrt_right**2%mod))
        print("sqrt_left: "+str(sqrt_left%N)+" sqr_left: "+str(sqrt_left**2%mod)+" prod_left_mod: "+str(prod_left_mod))
        z=((prod_left-prod_right)//N)//4
        y0=sqrt_left#%mod1

        y1=sqrt_right#%N
        testsqr=isqrt(disc)
        print("[i]Debug info: disc%N: "+str(disc%N)+" (disc): "+str(disc)+" jacobi_symbols: "+str(jacobi_symbols)+" prod_left: "+str(prod_left)+" prod_right: "+str(prod_right)+" z: "+str(z%N)+" ja: "+str(ja)+" y0: "+str(y0))#+" x: "+str(x)+" eq1: "+str(eq1_full%mod1))
        ...
        if testsqr**2 == disc:
            ...
            if factor_candidate not in (1, N):
                ...
                print("[SUCCESS]Factors are: "+str(factor_candidate)+" and "+str(other_factor))
                if wrap == 1:
                    print("Found a solution with modulus wrap around")
...
def solve_bits(matrix):
    n=p_amount*1+2+quad_char_amount
    lsmap = {lsb: 1 << lsb for lsb in range(n+1000)}
    m = len(matrix)
    marks = []
    cur = -1
    mark_mask = 0
...
def build_matrix(factor_base, smooth_nums,smooth_nums2,factors2,qchar):
    print("\n\n")
    fb_len = len(factor_base)
    fb_map = {val: i for i, val in enumerate(factor_base)}
    ind=1
    #factor_base.insert(0, -1)
    M2=[0]*(p_amount*1+2+quad_char_amount)
...
def QS(n,factor_list,sm,colist,sm2,flist2,qchar,z_list,disc_list,quad_base,mod):
    global p_amount
    ...
    if p_amount > 500:
        p_amount=500
    g_max_smooths=p_amount*1+2+quad_char_amount
    ...
    M2 = build_matrix(factor_list, sm,sm2,flist2,qchar)
    null_space=solve_bits(M2)
    f1,f2=extract_factors(n, sm, null_space,sm2,colist,z_list,disc_list,qchar,quad_base,mod)
    if f1 != 0:
...
def find_comb(lists,n,procnum,return_dict,hmap,quad_base):
    ...
    while z < z_max:
        ...
        z+=2
    test=QS(n,lists,ret_array[0],ret_array[1],ret_array[6],ret_array[7],ret_array[3],ret_array[5],ret_array[8],quad_base,mod)   
    return 0
