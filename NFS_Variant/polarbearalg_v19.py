...
def gen_comb(collected,mod,z,n,factor_base,ret_array,quad_base):
    ...
    for idx in enumerated_product(*enum):
        ...
        while i < 1: 
            ...
            if abs(test_square)**0.5%1 !=0:
                i+=1
                continue 
 
            print(" mod: "+str(mod)+" z: "+str(z)+" test_square: "+str(test_square)+" y0: "+str(y0)) 
            y1=(abs(mod*test_square))**0.5
            factor=gcd(y1+y0,n)
          ...
          #  if rem == 1 and rem2 == 1:
              ...
              #  q=0
               # while q < len(quad_base):
                    ...
                    #ja_ys=jacobi((disc_y1)%quad_base[q],quad_base[q])
                    #if ja_ys == -1:
                     #    qchar.append(-1)
                   # elif ja_ys != 0:
                    #    ys=tonelli((disc_y1)%quad_base[q],quad_base[q])
                #    if jacobi((n*4*z)%quad_base[q],quad_base[q]) == -1:
