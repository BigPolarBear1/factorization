...
p_mod_amount=30 #amount of primes in the modulus
   ...
   # ret_array[2].append(mod)
    ...
    for idx in enumerated_product(*enum):
        ...
        while i < 1:
            ...
            y0=n-eq2
            sqrt_start=sqrt
            while sqrt_start < n and sqrt !=0:
                
                factor=gcd(abs(y0+sqrt_start),n)
                if factor != 1 and factor != n:
                    if y0**2%n == sqrt_start**2%n:
                        print("Factored N: "+str(factor)+" and: "+str(n//factor)+" y0: "+str(y0)+" y1: "+str(sqrt_start))
                        sys.exit()
               # print(str(sqrt_start)+" "+str(y0)+" sqrt"
                sqrt_start+=sqrt
