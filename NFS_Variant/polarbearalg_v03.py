...
###notes: use with python3 polarbearalg_v03.py -key 4387, refer to paper for the math.
...
p_amount=5 #amount of primes in factor base
...
def gen_comb(collected,mod,z,n,factor_base,ret_array):
    ...
    for idx in enumerated_product(*enum):
        ...
        i=0
        while i < 100:  ###Idea: We can also increase the modulus on the quadratic coefficient... i'll need to experiment in the future
            ...
            factors2,rem2=factorise_fast(eq1_b_full,factor_base)
            if rem == 1 and rem2 == 1:
...
def find_comb(lists,n,procnum,return_dict,rstart,rstop,hmap):
    ...
    if len(ret_array[0])>0:
        ...
        while g < len(ret_array[0]):
            print("alg1: "+str(ret_array[0][g])+" y0: "+str(ret_array[1][g])+" factors1: "+str(ret_array[2][g])+" z: "+str(ret_array[3][g])+" mod: "+str(ret_array[4][g])+" root: "+str(ret_array[5][g])+" alg2: "+str(ret_array[6][g])+" factors2: "+str(ret_array[7][g]))
            g+=1
